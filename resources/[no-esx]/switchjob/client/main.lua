...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas una pistola con muncion para poderlo matar!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("~r~No hay nadie cerca para cojer!!")
			end
		else
			drawNativeNotification("~r~No hay nadie cerca para cojer!!")
		end
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Apreta [G] para soltar o [H] para matarlo!")
