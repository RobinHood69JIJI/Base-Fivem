...
local deformationMultiplier = 2.0				-- How much should the vehicle visually deform from a collision. Range 0.0 to 10.0 Where 0.0 is no deformation and 10.0 is 10x deformation. -1 = Don't touch
...
local damageFactorEngine = 0.5				-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
...
local damageFactorPetrolTank = 5.0				-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 64
...
local cascadingFailureThreshold = 260.0			-- Below this value, health cascading failure will set in
local engineSafeGuard = 110.0					-- Final failure value. Set it too high, and the vehicle won't smoke when disabled. Set too low, and the car will catch fire from a single bullet to the engine. At health 100 a typical car can take 3-4 bullets to the engine before catching fire.
