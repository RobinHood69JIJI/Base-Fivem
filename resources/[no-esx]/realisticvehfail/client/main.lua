...
RegisterNetEvent("ines:repair_full")
AddEventHandler("ines:repair_full", function ()
	local ped = GetPlayerPed(-1)
	vehicle = GetVehiclePedIsIn(ped, false)
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~r~El vehiculo esta demasiado dañado para repararlo.")
			end
		else
		...
		notification("~y~Debes estar dentro de un vehiculo para reparar el vehiculo.")
	end
end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification("~r~No tienes privilegios para reparar el vehiculo.")
end)
