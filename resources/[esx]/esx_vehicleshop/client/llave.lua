...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 303)) then
		...
		notowned = 0
		if #cars == 0 then
			TriggerEvent("pNotify:SendNotification", {text = "No hay ningún vehículo cerca.", type = "alert", timeout = 5000, layout = "centerLeft"})
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							TriggerEvent("pNotify:SendNotification", {text = "Has cerrado tu vehículo.", type = "error", timeout = 5000, layout = "centerLeft"})
						...
						elseif lock == 2 then
							...
							TriggerEvent("pNotify:SendNotification", {text = "Has abierto tu vehículo.", type = "success", timeout = 5000, layout = "centerLeft"})
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
