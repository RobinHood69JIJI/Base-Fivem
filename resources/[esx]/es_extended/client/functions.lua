...
ESX.Keys = { ["ESC"] = 322, ["F1"] = 288, ["F2"] = 289, ["F3"] = 170, ["F5"] = 166, ["F6"] = 167, ["F7"] = 168, ["F8"] = 169, ["F9"] = 56, ["F10"] = 57, ["~"] = 243, ["1"] = 157, ["2"] = 158, ["3"] = 160, ["4"] = 164, ["5"] = 165, ["6"] = 159, ["7"] = 161, ["8"] = 162, ["9"] = 163, ["-"] = 84, ["="] = 83, ["BACKSPACE"] = 177, ["TAB"] = 37, ["Q"] = 44, ["W"] = 32, ["E"] = 38, ["R"] = 45, ["T"] = 245, ["Y"] = 246, ["U"] = 303, ["P"] = 199, ["["] = 39, ["]"] = 40, ["ENTER"] = 18, ["CAPS"] = 137, ["A"] = 34, ["S"] = 8, ["D"] = 9, ["F"] = 23, ["G"] = 47, ["H"] = 74, ["K"] = 311, ["L"] = 182, ["LEFTSHIFT"] = 21, ["Z"] = 20, ["X"] = 73, ["C"] = 26, ["V"] = 0, ["B"] = 29, ["N"] = 249, ["M"] = 244, [","] = 82, ["."] = 81, ["LEFTCTRL"] = 36, ["LEFTALT"] = 19, ["SPACE"] = 22, ["RIGHTCTRL"] = 70, ["HOME"] = 213, ["PAGEUP"] = 10, ["PAGEDOWN"] = 11, ["DELETE"] = 178, ["LEFT"] = 174, ["RIGHT"] = 175, ["TOP"] = 27, ["DOWN"] = 173, ["NENTER"] = 201, ["N4"] = 108, ["N5"] = 60, ["N6"] = 107, ["N+"] = 96, ["N-"] = 97, ["N7"] = 117, ["N8"] = 61, ["N9"] = 118}
...
ESX.PlayerHasJob = function(Playerdata,job)
    return (Playerdata.job ~= nil and Playerdata.job.name == job)
end

ESX.GetKeys = function()
	return ESX.Keys
end

ESX.DrawText3D = function(x,y,z,textInput,fontId,scaleX,scaleY,opacity)
...
ESX.UI.HUD.SetMission = function(text)
	SendNUIMessage({
		action  = 'activity_on',
		text = text
	})
...
ESX.UI.HUD.SetMissionOff = function()
	SendNUIMessage({
		action  = 'activity_off'
	})
end
...
ESX.UI.Menu.Open = function(type, namespace, name, data, submit, cancel, change, close)
	...
	menu.setDescription = function(val)
		menu.data.description = val
	end
	...
	if DoesEntityExist(vehicle) then
		...
		if(Config.SerializeCarDamage)then
			if(props.condition)then
				if props.condition.totalkm ~= nil then
					DecorSetFloat(vehicle, 'TOTALKM', tonumber(props.condition.totalkm))
				else
					DecorSetFloat(vehicle, 'TOTALKM', 0)
				end
				if props.condition.bodyHealth ~= nil then
					SetVehicleBodyHealth(vehicle, props.condition.bodyHealth+0.00)
				end
				
				if props.condition.engineHealth ~= nil then
					SetVehicleEngineHealth(vehicle,props.condition.engineHealth+0.00)
				end  
				
				if props.condition.fuelLevel ~= nil then
					SetVehicleFuelLevel(vehicle,props.condition.fuelLevel+0.00)
				end
				if props.condition.petrolTankHealth ~= nil then
					SetVehiclePetrolTankHealth(vehicle,props.condition.petrolTankHealth+0.00)
				end
				if props.condition.windows then
					for i, window_state in pairs(props.condition.windows) do
						if window_state == false then
							--print('SmashVehicleWindow '.. i)
							SmashVehicleWindow(vehicle, tonumber(i))
						end
					end
				end
				if props.condition.doors then
					for i, door_state in pairs(props.condition.doors) do
						if not door_state then
							--print('SetVehicleDoorBroken '.. i)
							SetVehicleDoorBroken(vehicle, tonumber(i), true)
						end
					end
				end
				if props.condition.tyres then
					for i, tyre_state in pairs(props.condition.tyres) do
						if tyre_state < 2 then
							--print('SetVehicleTyreBurst ' .. i)
							SetVehicleTyreBurst(vehicle, tonumber(i), tyre_state == 0, 1000.0)
						end
					end
				end
			end
...
ESX.ShowInventory = function()
	...
	if isInMision then
		table.insert(elements, {label = '--Cancelar misión--', value = 'cancel_mission'})
	end	
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory', {
		...
		menu.setDescription("Inventario")
		menu.close()
		local player, distance = ESX.Game.GetClosestPlayer()
		elements = {}

		if data.current.value == 'cancel_mission' then
			print('mission cancelada')
		end
