(() => {
	...
	ESX.disableHUDElement = function(name)	{
		for (let i = 0; i < ESX.HUDElements.length; i++) {
			if (ESX.HUDElements[i].name == name) {
				ESX.HUDElements[i].disabled = true;
			}
		}
	};

	ESX.enableHUDElement = function(name)	{
		for (let i = 0; i < ESX.HUDElements.length; i++) {
			if (ESX.HUDElements[i].name == name) {
				ESX.HUDElements[i].disabled = false;
			}
		}
	};
	...
	ESX.inMission = function(text)
	{
		$('#missionName').html(text);
		$('#mission').show();
	};
	ESX.offMission = function()
	{
		$('#missionName').html('');
		$('#mission').hide();
	};
	...
	window.onData = (data) => {
		switch (data.action) {
			...
			case 'disableHUDElement': {
				MSX.disableHUDElement(data.name);
				break;
			}

			case 'enableHUDElement': {
				ESX.enableHUDElement(data.name);
				break;
			}
			
			case 'activity_on':{
				ESX.inMission(data.text);
				break;
			}
			case 'activity_off':{
				ESX.offMission();
