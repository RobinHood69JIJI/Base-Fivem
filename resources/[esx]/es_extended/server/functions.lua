ESX.Trace = function(msg)
	if Config.EnableDebug then
		print(('^5[es_extended] [^2TRACE^7] %s^7'):format(msg))
	end
end
...
ESX.RegisterCommand = function(name, group, cb, allowConsole, suggestion)
	...
	if ESX.RegisteredCommands[name] then
		print(('^3[es_extended] An command "%s" is already registered, overriding command^7'):format(name))
	...
	RegisterCommand(name, function(playerId, args, rawCommand)
		...
		if not command.allowConsole and playerId == 0 then
			print(('^5^3[Extended]^0 %s^7'):format(_U('commanderror_console')))
		else
			...
			if error then
				if playerId == 0 then
					print(('^5^3[Extended]^0 %s^7'):format(error))
				else
				...
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						print(('^5^3[Extended]^0 %s^7'):format(msg))
					else
...
ESX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('^5[es_extended] [^2WARNING^5] Server callback "%s" does not exist. Make sure that the server sided file really is loading, an error in that file might cause it to not load.^7'):format(name))
	end
end
...
ESX.SavePlayer = function(xPlayer, cb)
	...
	Async.parallel(asyncTasks, function(results)
		print(('^3[Extended]^0 Jugador guardado: ^1%s^0.'):format(xPlayer.getName()))
...
ESX.SavePlayers = function(cb)
	...
	Async.parallelLimit(asyncTasks, 8, function(results)
		print(('^3[Extended] ^1%s^0 jugadores guardados^0.'):format(#xPlayers))
		if cb then
			cb()
		end
	end)
...
ESX.GetPlayerJobFromId = function(source)
	if ESX.Players[tonumber(source)] then
		return  ESX.Players[tonumber(source)].job
	else
		return nil
	end
end
...
ESX.GetPlayerNameFromId = function(source)
	if ESX.Players[tonumber(source)] then
		return  ESX.Players[tonumber(source)].name
	else
		return nil
	end
end

ESX.GetPlayerBasicFromId = function(source)
	if ESX.Players[tonumber(source)] then
...
ESX.GetSetFromId = function(source)
	if ESX.Players[tonumber(source)] then
		local xPlayer = ESX.Players[tonumber(source)]
		local getSet = {
			source = xPlayer.source,
...
ESX.GetPlayerCoordsFromId = function(source)
	if ESX.Players[tonumber(source)] then
		return  ESX.Players[tonumber(source)].coords
	else
		return nil
	end
end
...
ESX.GetPlayersByJob = function(job)
	local sources = {}
