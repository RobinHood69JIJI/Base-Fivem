...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elements = {
			...
			{label = 'Alertas Policiales', value = 'alertas_policiales'}
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						...
						ExecuteCommand("me Le pone/quita las esposas")
					elseif action == 'drag' then
						TriggerServerEvent('esx_policejob:drag', GetPlayerServerId(closestPlayer))
						ExecuteCommand("me Le escolta/Le deja de escoltar")
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_policejob:putInVehicle', GetPlayerServerId(closestPlayer))
						ExecuteCommand("me Lo mete en el coche")
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_policejob:OutVehicle', GetPlayerServerId(closestPlayer))
						ExecuteCommand("me Lo saca del coche")
					elseif action == 'fine' then
						OpenFineMenu(closestPlayer)
						ExecuteCommand("e libreta")
						ExecuteCommand("me Redacta las multas correspondientes")
		...
		elseif data.current.value == 'alertas_policiales' then
            local elements = {
                {label = "[DEFCON] Nivel 1", value = 'green'},
                {label = "[DEFCON] Nivel 2", value = 'yellow'},
                {label = "[DEFCON] Nivel 3", value = 'orange'},
                {label = "[DEFCON] Nivel 4", value = 'red'},
                {label = "[DEFCON] Quitar Nivel", value = 'close'}
            }

            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'alertas_policiales', {
                title    = "Alertas",
                align    = 'bottom-right',
                elements = elements
            }, function(data2, menu2)
                ...
                if action then
					if action == 'close' then
                        TriggerServerEvent('alertas:disable')
					else
                        TriggerServerEvent('alertas:active', action)    
                    end
                    menu2.close()
                end
            end, function(data2, menu2)
